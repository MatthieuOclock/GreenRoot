...
const stripePromise = loadStripe('pk_test_51PM77gHRY0RGHhfcc3yltn0UQxor4nbXVuwIU2OBjzRA3TkSUx5G3ioywHLbK1qZasZoyatbz9wc35NghOalG7Rk00Y7jlQmhi');
...
const Payment = () => {
    ...
    const totalAmount = panier.reduce((total, tree) => total + tree.price * tree.quantity, 0);
    ...
    return (
        <Elements stripe={stripePromise}>
            <h1>Paiement: <span>{totalAmount}</span></h1>
            <div className="payment-container">
                <div className="produits">
                    ...
                    <form method="POST" action="#" className='form'>
                        ...
                        <select name="payment-method" value={selectedPaymentMethod} onChange={handlePaymentMethodChange}>
                            <option value="card">Carte bancaire</option>
                            <option value="cheque">Chèque</option>
                            <option value="bank-transfer">Virement bancaire</option>
                        </select>
                    </form>
                    {selectedPaymentMethod === 'card' && <StripePaymentForm clientSecret={clientSecret} panier={panier} />}
                    {selectedPaymentMethod === 'cheque' && <ChequePaymentInfo />}
                    {selectedPaymentMethod === 'bank-transfer' && <BankTransferPaymentInfo />}
                </div>
            </div>
        </Elements>
    );
...
const StripePaymentForm = ({ clientSecret, panier }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <CardElement />
            <button type="submit" disabled={!stripe}>Procédez au paiement</button>
        </form>
    );
};
