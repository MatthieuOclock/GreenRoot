...
const ChefDeProjet = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            const url = isAddingNew ? `${process.env.URL_API}campain` : `${process.env.URL_API}campain / ${editingCampain}`;
            ...
            if (response.ok) {
                ...
                if (isAddingNew) {
                    ...
                    setCampains(campains.map(campain => campain.id === editingCampain ? updatedCampain : campain));
                    setEditingCampain(null);
                }
                setFormValues({
                    name: '',
        ...
        if (window.confirm('Voulez-vous vraiment supprimer cette campagne ?')) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.error('Erreur lors de la suppression de la campagne:', response.statusText);
                }
            } catch (error) {
        ...
        <div>
            ...
            <ul>
                {campains.map(campain => (
                    <li key={campain.id}>
                        ...
                        <button onClick={() => handleDeleteClick(campain.id)}>Supprimer</button>
                    </li>
                ))}
            </ul>
            ...
            {(editingCampain || isAddingNew) && (
                <form onSubmit={handleFormSubmit}>
                    <h3>{isAddingNew ? 'Ajouter une nouvelle campagne' : 'Modifier la campagne'}</h3>
                    <label>
                        Nom:
                        <input
                            type="text"
