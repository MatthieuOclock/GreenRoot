...
const Administrateur = () => {
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        const confirmMessage = editingUser ? 'Voulez-vous vraiment modifier cet utilisateur ?' : 'Voulez-vous vraiment ajouter cet utilisateur ?';
        if (window.confirm(confirmMessage)) {
            try {
                const token = localStorage.getItem('token');
                const method = editingUser ? 'PUT' : 'POST';
                const url = editingUser ? `${process.env.URL_API}user/${editingUser}` : `${process.env.URL_API}user`;
        ...
        <div>
            ...
            <ul>
                {users.map(user => (
                    <li key={user.id}>
                        ...
                        <button onClick={() => handleDeleteClick(user.id)}>Supprimer</button>
                    </li>
                ))}
            </ul>
            ...
            {(editingUser || isAddingNew) && (
                <form onSubmit={handleFormSubmit}>
                    ...
                    <br />
                    <button type="submit">{editingUser ? 'Enregistrer les modifications' : 'Ajouter'}</button>
                </form>
            )}
            <ChefDeProjet />
        </div>
